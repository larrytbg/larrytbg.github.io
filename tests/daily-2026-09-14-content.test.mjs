...
const expected = new Map([
  ["daily/03", { title: "让标题更吸引人不等于制造标题党：双重引导模型尝试同时守住忠实度", source: "https://www.nature.com/articles/s41598-026-71420-6", sourceDate: "2026-09-13" }],
  ["finance/03", { title: "金融预测没有万能算法：模型表现取决于市场结构与数据关系", source: "https://www.nature.com/articles/s41598-026-71418-0", sourceDate: "2026-09-13" }],
  ["health/03", { title: "大型匹配队列发现术前GLP-1受体激动剂暴露与较低的持续阿片处方相关", source: "https://www.nature.com/articles/s41598-026-71924-1", sourceDate: "2026-09-13" }],
  ["papers/03", { title: "加拿大高北极冰川退缩序列显示细菌与真菌群落以不同节奏接替", source: "https://www.nature.com/articles/s41598-026-71128-7", sourceDate: "2026-09-13" }],
  ["financial-literacy/01", { title: "金融包容不能只看信贷规模：乌干达长期数据呈现短期与长期方向差异", source: "https://link.springer.com/article/10.1007/s43621-026-04717-1", sourceDate: "2026-09-13" }],
]);
const targets = [...expected.keys()].sort();

test("9月14日只发布五篇通过事实复核的真实新增", async () => {
  ...
  assert.deepEqual(audit.scope, { daily: 1, finance: 1, health: 1, papers: 1, ted: 0, "financial-literacy": 1 });
...
test("首页、审计、高亮、镜像和110篇日期台账同步", async () => {
  ...
  assert.ok(home.includes("2026年9月14日"));
...
test("公开产物不泄露私有路径或内部任务身份", async () => {
  const files = [
    ...
    ...targets.map((target) => `site/column/${target}/index.html`), "scripts/publish-2026-09-14.mjs",
  ];
