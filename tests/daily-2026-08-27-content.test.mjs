...
const audit = JSON.parse(await readFile("data/update-audit-2026-08-27.json", "utf8"));
const targets = audit.articles.map((entry) => entry.target);

test("8月27日审计只包含20篇复核通过的实质更新", () => {
  assert.equal(audit.date, "2026-08-27");
  assert.equal(audit.substantivelyUpdated, 20);
  assert.equal(audit.articles.length, 20);
...
test("公开审计副本与数据审计完全一致", async () => {
  const publicAudit = JSON.parse(await readFile("site/audit/update-2026-08-27.json", "utf8"));
  assert.deepEqual(publicAudit, audit);
});

test("20篇详情页标题、来源、七段正文和长度都满足发布要求", async () => {
  for (const entry of audit.articles) {
    ...
    assert.match(html, /data-substantive-update="2026-08-27"/);
    ...
    assert.ok(text.length >= 1200, `${entry.target} 正文只有 ${text.length} 字符`);
    assert.ok(text.length <= 1800, `${entry.target} 正文达到 ${text.length} 字符`);
  }
});

test("与更新前基线相比恰好20篇正文发生语义变化", async () => {
  const baseline = JSON.parse(await readFile("data/article-baseline-2026-08-27.json", "utf8"));
  const changed = [];
...
test("TED只更新两篇且日期台账只标记本批20篇", async () => {
  assert.deepEqual(targets.filter((target) => target.startsWith("ted/")).sort(), ["ted/01", "ted/02"]);
  ...
  const updatedToday = Object.entries(ledger.articles)
    .filter(([, entry]) => entry.updatedOn === "2026-08-27")
    .map(([target]) => target)
    .sort();
  ...
  for (const entry of audit.articles) {
    ...
    assert.equal(ledger.articles[entry.target].updatedOn, "2026-08-27");
