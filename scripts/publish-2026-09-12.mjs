...
const dateCn = "2026年9月12日";
const checkedAt = "2026-09-12 07:20（北京时间）";
const version = "33";
...
const mappings = new Map([
  ["daily-20260911-hunga-caldera-collapse", { target: "daily/01" }],
  ["finance-20260911-california-psps-investment", { target: "finance/01" }],
  ["health-20260911-teclistamab-phase2", { target: "health/01" }],
  ["papers-20260911-nd-manganese-catalyst", { target: "papers/01" }],
  ["logic-20260911-universe-topology-review", { target: "logic/01" }],
]);
...
const updates = drafts.map((item) => {
  ...
  const approved = initialReviewById.get(item.id)?.verdict === "PASS";
...
const columnNames = { daily: "每日资讯", finance: "金融", health: "身体健康", papers: "论文研究", logic: "逻辑" };
const accents = { daily: "accent-vermilion", finance: "accent-gold", health: "accent-green", papers: "accent-indigo", logic: "accent-purple" };
...
const note = `<p class="today-update-note">9月12日完成5篇真实实质更新：每日资讯、金融、身体健康、论文研究和轮换的逻辑栏目各1篇；TED过去24小时没有核准到官方新演讲，未用旧内容补数。${reviewSentence}。<a href="/audit">查看逐篇审计 →</a></p>`;
...
const scope = { daily: 1, finance: 1, health: 1, papers: 1, ted: 0, logic: 1 };
...
const audit = {
  ...
  scope,
  sourceScan: { candidates: 8, readyFromWorker: 4, hold: 4, workerReadyRejected: 3, controllerExpanded: true, selected: 5, note: "工作任务READY中2条与前一日重复、1条超出24小时时窗，均未采用。" },
  tedScan: { scanned: true, newItems: 0, updated: 0, note: "过去24小时未核准到TED官方新增内容，未用旧演讲补数。" },
  rotation: { column: "logic", updated: 1, note: "逻辑轮换栏目用宇宙拓扑综述训练区分未发现证据、排除范围与确认命题。" },
...
const auditPage = `<!DOCTYPE html><html lang="zh-CN"><head><meta charset="utf-8"/><meta name="viewport" content="width=device-width,initial-scale=1"/><link rel="stylesheet" href="/assets/index-CVB57ELS.css"/><link rel="stylesheet" href="/assets/daily-highlights.css"/><title>9月12日更新审计 · 自学总站</title></head><body><header class="site-header"><div class="shell header-inner"><a href="/" class="brand"><span class="brand-mark">知</span><span>自学总站</span></a><nav class="main-nav"><a href="/">全部专栏</a><a href="/ski-training">滑雪训练</a><a href="/archive">每日归档</a></nav><div class="live-status"><span></span> 2026.09.12 · 每日更新 ${version}</div></div></header><main class="shell audit-page"><header class="directory-header"><p>${dateCn} · 真实更新审计</p><h1>5篇真实实质更新，按新鲜度与质量发布</h1><p>每日资讯、金融、身体健康、论文研究和逻辑各1篇。${reviewSentence}。</p></header><section class="audit-summary"><h2>质量说明</h2><p>5篇均来自9月11日发布的原始论文页，日期、主体、数字、研究类型、因果与边界经过独立事实复核；TED没有官网新演讲，没有用旧稿补数。</p></section><section class="audit-list"><h2>逐篇变更</h2><ul>${rows}</ul></section></main><footer class="site-footer"><div class="shell footer-inner"><p>自学总站 · 长期自学知识库</p><p>${dateCn} · 每日更新 ${version}</p></div></footer></body></html>`;
...
archive = archive.replace(/<article class="archive-entry" data-archive-date="2026-09-12">[\s\S]*?<\/article>/, "");
const archiveEntry = `<article class="archive-entry" data-archive-date="2026-09-12"><div class="archive-date"><strong>2026.09.12</strong><span>每日更新 ${version} · 5篇真实实质更新</span></div><div class="archive-content"><h2>${dateCn}</h2><p>本批按质量优先更新5篇：${reviewSentence}；TED没有合格新增，未用旧稿补数。</p><ul><li><span>更新审计</span><a href="/audit">查看5篇逐条审计 →</a></li></ul></div></article>`;
