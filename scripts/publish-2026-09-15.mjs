...
const dateCn = "2026年9月15日";
const checkedAt = "2026-09-15 07:27（北京时间）";
const version = "36";
...
const mappings = new Map([
  ["daily-20260914-rangeland-adaptation", { target: "daily/04" }],
  ["finance-20260914-dual-chain-credit", { target: "finance/04" }],
  ["health-20260914-multimodal-bci", { target: "health/04" }],
  ["papers-20260914-offshore-noise-plankton", { target: "papers/04" }],
  ["management-20260914-leadership-perceptions", { target: "management/04" }],
]);
...
const updates = drafts.map((item) => {
  ...
  const badStructure = plainLength < 1000
    || (item.bodyHtml.match(/<h2>/g) ?? []).length < 8
...
const columnNames = { daily: "每日资讯", finance: "金融", health: "身体健康", papers: "论文研究", management: "职场管理学" };
const accents = { daily: "accent-vermilion", finance: "accent-gold", health: "accent-green", papers: "accent-indigo", management: "accent-gold" };
...
const note = `<p class="today-update-note">9月15日完成5篇真实实质更新：每日资讯、金融、身体健康、论文研究和轮换的职场管理学栏目各1篇；TED过去24小时没有核准到官方新演讲，未用旧内容补数。${reviewSentence}。<a href="/audit">查看逐篇审计 →</a></p>`;
...
const scope = { daily: 1, finance: 1, health: 1, papers: 1, ted: 0, management: 1 };
...
const audit = {
  ...
  scope,
  sourceScan: { candidates: 1, readyFromWorker: 0, hold: 1, workerReadyRejected: 0, workerHoldPromoted: 0, controllerExpanded: true, selected: 5, note: "来源扫描任务交付1条HOLD候选，因官方页面缺少可核准的日级发布日期未采用；总控扩大检索并回到5个A级原始出版页核验后采用。" },
  freshness: { within24Hours: 5, currentIssueWithoutExactDay: 0, note: "5篇均来自2026年9月14日发布的原始期刊页面，并在北京时间9月15日更新窗口内核准。" },
  tedScan: { scanned: true, newItems: 0, updated: 0, note: "TED官网候选缺少可核准的日级发布日期，无法证明过去24小时真实新增；未用旧演讲补数。" },
  rotation: { column: "management", updated: 1, note: "职场管理学轮换栏目用于训练对问卷感知、样本边界与绩效因果之间的区分。" },
  draftSummary: { workerRounds: 2, workerResult: "both deliveries failed structure and duplicate-paragraph quality gates", controllerTakeover: true, note: "总控依据已核验证据完成逐篇非模板化终稿；两轮工作稿均未进入网站。" },
  reviewSummary,
...
const auditPage = `<!DOCTYPE html><html lang="zh-CN"><head><meta charset="utf-8"/><meta name="viewport" content="width=device-width,initial-scale=1"/><link rel="stylesheet" href="/assets/index-CVB57ELS.css"/><link rel="stylesheet" href="/assets/daily-highlights.css"/><title>9月15日更新审计 · 自学总站</title></head><body><header class="site-header"><div class="shell header-inner"><a href="/" class="brand"><span class="brand-mark">知</span><span>自学总站</span></a><nav class="main-nav"><a href="/">全部专栏</a><a href="/ski-training">滑雪训练</a><a href="/archive">每日归档</a></nav><div class="live-status"><span></span> 2026.09.15 · 每日更新 ${version}</div></div></header><main class="shell audit-page"><header class="directory-header"><p>${dateCn} · 真实更新审计</p><h1>5篇真实实质更新，按新鲜度与质量发布</h1><p>每日资讯、金融、身体健康、论文研究和职场管理学各1篇。${reviewSentence}。</p></header><section class="audit-summary"><h2>质量说明</h2><p>5篇均来自9月14日发布的原始期刊页面。全部条目已核对主体、文章类型、样本、数字、事实、因果与边界；TED没有核准到过去24小时真实新增演讲。</p></section><section class="audit-list"><h2>逐篇变更</h2><ul>${rows}</ul></section></main><footer class="site-footer"><div class="shell footer-inner"><p>自学总站 · 长期自学知识库</p><p>${dateCn} · 每日更新 ${version}</p></div></footer></body></html>`;
...
archive = archive.replace(/<article class="archive-entry" data-archive-date="2026-09-15">[\s\S]*?<\/article>/, "");
const archiveEntry = `<article class="archive-entry" data-archive-date="2026-09-15"><div class="archive-date"><strong>2026.09.15</strong><span>每日更新 ${version} · 5篇真实实质更新</span></div><div class="archive-content"><h2>${dateCn}</h2><p>本批按质量优先更新5篇：${reviewSentence}；TED没有合格新增，未用旧稿补数。</p><ul><li><span>更新审计</span><a href="/audit">查看5篇逐条审计 →</a></li></ul></div></article>`;
